...
# Convert ophtal.sim into mcmc.list for processing with CODA package
...
# Running JAGS (with the same model as previously):
jags <- jags.model(file = "ophtalmology_model.txt",
                   data = my.data,
                   inits = my.inits,
                   n.chains = 2)
...
#### 2.3 Longer model with thinning, burnin, centering (Question 2) ####
cat("model
  {
    for (i in 1:N){
      lutein_2003[i] ~ dnorm(mu[i],tau)
...
gelman.plot(ophtal.mcmc_complex[,1], main = 'beta0', ask = FALSE)
gelman.plot(ophtal.mcmc_complex[,2], main = 'beta1', ask = FALSE)
gelman.plot(ophtal.mcmc_complex[,3], main = 'sigma2', ask = FALSE)
...
# Calculate posterior probability of corrected serum-lutein >5 for each new observation
for (j in 1:N_new) {
